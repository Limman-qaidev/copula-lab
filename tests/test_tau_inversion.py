...
def test_rho_matrix_from_tau_gaussian_matches_formula() -> None:
    tau = np.array([[1.0, 0.3, -0.2], [0.3, 1.0, 0.1], [-0.2, 0.1, 1.0]])
    corr = rho_matrix_from_tau_gaussian(tau)
    expected = np.sin(0.5 * np.pi * tau)
    np.fill_diagonal(expected, 1.0)
    assert np.allclose(corr, expected)


def test_rho_matrix_from_tau_student_t_matches_gaussian_mapping() -> None:
    tau = np.array([[1.0, 0.25], [0.25, 1.0]])
    corr_student = rho_matrix_from_tau_student_t(tau)
    corr_gaussian = rho_matrix_from_tau_gaussian(tau)
    assert np.allclose(corr_student, corr_gaussian)
